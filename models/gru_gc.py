...
class GRU_GC(object):
    def __init__(self, gru_opt, training_opt):
        ...
        self.weight_decay = getattr(training_opt, "weight_decay", 1e-7)  # 기본값 추가
        self.theta = getattr(training_opt, "theta", 0.09)  # 기본값 추가
        ...
        for k in range(self.num_channel):
            ...
            for i in range(self.num_channel):
                ...
                for x_idx in channel_set:
                    ...
                    # 데이터 가공
                    train_subset = [(inputs[:, :, tmp_set], targets[:, k].unsqueeze(-1)) for inputs, targets in train_loader]
                    val_subset = [(inputs[:, :, tmp_set], targets[:, k].unsqueeze(-1)) for inputs, targets in val_loader]
                    train_loader_sub = DataLoader(train_subset, batch_size=train_loader.batch_size, shuffle=True)
                    val_loader_sub = DataLoader(val_subset, batch_size=val_loader.batch_size, shuffle=False)
                    ...
                    # 훈련
                    train(model, train_loader_sub, val_loader_sub, criterion, optimizer,
                    ...
                    val_loss = 0.0
                    with torch.no_grad():
                        for inputs, targets in val_loader_sub:
            ...
            with torch.no_grad():
                ...
                for inputs, targets in val_loader:
                    inputs, targets = inputs.to(device), targets.to(device)
                    inputs = inputs[:, :, input_set]
                    targets = targets[:, k].unsqueeze(-1)
                    outputs = model(inputs)
